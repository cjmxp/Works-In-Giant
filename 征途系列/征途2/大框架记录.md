...
10. `IBitmap.hpp,IBitmaps.hpp,IBitmapList.h`这三个是属于图片的的处理信息，里面有图片的BlendType，像素点编码格式，图片group种类，AlphaType，图片中的图像块信息（宽度，高度，像素格式），这部分有空可以联系一些图片的知识看看。（我估计是没有机会再来研究这部分代码了）
### IO

1. 内含文件：``
### math
1. 内含文件：`MathEx.h`
2. 里面是二维点stVector2,线段stLine（里面有一个一般式函数，可以用来判断点是否在线段上），
### memory
### net
### other
### register
